...
## 🎮 Cara Memainkan
1. Buka file `index.html` melalui browser komputer atau perangkat seluler Anda.
2. Klik tombol **START SURVIVAL** untuk memulai permainan dan mengaktifkan mesin audio.
3. **Kontrol PC / Laptop:**
   * Tombol `Panah Kiri` / `A` & `Panah Kanan` / `D`: Menggeser posisi meriam markas.
   * Tombol `Spasi`: Menembakkan proyektil suar api.
4. **Kontrol Mobile / Tablet:**
   * Gunakan tombol virtual panah (`◀` / `▶`) dan tombol bulat (`FIRE`) di bagian bawah layar.
5. Hancurkan semua gelombang monster es sebelum mereka berhasil membekukan markas Anda!
